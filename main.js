	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		(function initScrollAnimations() {
				...
				function (entries) {
					entries.forEach(function (entry) {
						if (entry.isIntersecting) {
							...
							// Для элементов со счётчиком запускаем анимацию подсчёта
			...
			var animatedElements = document.querySelectorAll(
				".animate-fade-in, .animate-fade-up, .animate-fade-down, .text-card, .stat, .faq__item, .news-card, .compare__col"
			);
		...
		// ===== Анимация подсчёта чисел в статистике =====
		function animateCounter(statElement) {
			var valueElement = statElement.querySelector(".stat__value");
			if (!valueElement || valueElement.dataset.animated === "true") return;

			var text = valueElement.textContent.trim();
			var numberMatch = text.match(/[\d,]+/);
			if (!numberMatch) return;

			var numberStr = numberMatch[0].replace(/,/g, "");
			var targetValue = parseInt(numberStr, 10);
			if (isNaN(targetValue)) return;

			// Определяем суффикс (всё после числа)
			var suffix = text.replace(numberMatch[0], "").trim();
			var prefix = text.substring(0, text.indexOf(numberMatch[0]));

			valueElement.dataset.animated = "true";
			var startValue = 0;
			var duration = 2000; // 2 секунды
			var startTime = null;
			...
			function updateCounter(timestamp) {
				...
				valueElement.textContent = prefix + formattedValue + (suffix ? " " + suffix : "");
